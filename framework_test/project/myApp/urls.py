...
urlpatterns = [
    ...
    url(r'^regist/$', views.regist, name='1'),  # regist需求转至views中的regist函数,下同
    ...
    url(r'^login/5/(\d+)$', view_firstWeek.launch_homework, name='launch'),  # firstWeek的模板及view_firstWeek的视图
    ...
    url(r'^login/8/(\d+)/(\d+)$', view_firstWeek.check_teacher_homework, name='7'),  # 老师查看作业完成情况
    url(r'^login/8/1/1/(\d+)$', view_firstWeek.teacher_alter_homework, name='8'),  # 老师修改对应作业
    url(r'^login/8/1/2/(\d+)$', view_firstWeek.delete_teacher_homework, name='9'),  # 老师删除对应作业
    url(r'^login/9/2/(\d+)/(\d+)$', view_firstWeek.check_submission_homework, name='10'),  # 老师批改学生列表
    url(r'^login/9/2/2/(\d+)/(\d+)$', view_firstWeek.check_common_student_the_homework, name='11'),  # 老师查看正确或订正正确且未迟交的学生名单
    url(r'^login/9/(\d+)/(\d+)$', view_firstWeek.check_feedback_homework, name='12'),  # 老师查看反馈学生列表
    url(r'^login/9/1/1/(\d+)/(\d+)$', view_firstWeek.check_redo_homework, name='13'),  # 老师查看未订正学生列表
    url(r'^login/9/1/2/(\d+)/(\d+)$', view_firstWeek.check_not_submitted_homework, name='14'),  # 老师查看未提交学生列表
    ...
    url(r'^login/14/(\d+)/(\d+)/(\d+)$', view_firstWeek.check_student_finished_homework_html3_handInTime, name='15'),  # 学生查看已提交作业html3（默认提交日期排序）
    url(r'^login/14/1/(\d+)/(\d+)/(\d+)$', view_firstWeek.check_student_finished_homework_html3_createtime, name='16'),  # 学生查看已提交作业html3（改为发布日期排序）
    ...
    url(r'^login/14/1/1/(\d+)/(\d+)/(\d+)$', view_firstWeek.feedback_homework, name='17'),  # 学生反馈作业
    ...
    url(r'^login/22/(\d+)/(\d+)/(\d+)$', view_secondWeek.check_unfinished_objective_item_html3_sorted_by_deadline, name='18'),  # 学生查看未提交客观题html3（默认截止日期排序）
    url(r'^login/22/1/(\d+)/(\d+)/(\d+)$', view_secondWeek.check_unfinished_objective_item_html3_sorted_by_createtime, name='19'),  # 学生查看未提交客观题html3（改为发布日期排序）
    ...
    url(r'^login/download/(?P<file_name>.*)/$', view_firstWeek.download, name='20'),  # 下载作业
]
