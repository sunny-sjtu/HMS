...
class UserForm_launch_homework(forms.Form):  # 发布作业信息表单
    homework_content = forms.CharField(label='zynr', max_length=50)
    deadline_days = forms.IntegerField(label='jth')
...
class UserForm_correct_homework(forms.Form):  # 批改作业表单
    ...
    isCorrect = forms.CharField(required=False, label='批改作业', max_length=200)
        ...
        if userform.is_valid():
            ...
            for teacher_grade in teacher_gradeList:
                ...
                for student in studentList:
                    ...
                    notice_content = "zyxs" + homework_content
            ...
            return render_to_response('myApp/webpage/launch_homework_1.html')
    else:
...
def submit_student_homework(request, num):  # 学生提交作业
    if request.method == 'POST':
        ...
        if userform.is_valid():
            ...
            if not myFile:
                return render_to_response('myApp/webpage/submit_student_homework_1.html')
            ...
            return render_to_response('myApp/webpage/submit_student_homework_2.html')
    else:
...
def correct_homework(request, num):  # 老师批改作业
    ...
    if s_homework.isComplete == False:
        return render_to_response('myApp/webpage/correct_homework_1.html')
        ...
        if request.method == 'POST':
            ...
            if userform.is_valid():
                ...
                if teacher_comment == "":
                    return render_to_response('myApp/webpage/correct_homework_2.html')

                else:
                    Homework1.objects.filter(pk=num).update(tcomment=teacher_comment, iscorrect=isCorrect,
                                                            iscommented=True)
                    return render_to_response('myApp/webpage/correct_homework_3.html')
...
def feedback_homework(request, num1, num2, num3):  # 学生反馈作业
    if request.method == 'POST':
        ...
        if userform.is_valid():
            ...
            if not myFile:
                return render_to_response('myApp/webpage/feedback_homework_1.html')
            ...
            return render_to_response('myApp/webpage/feedback_homework_2.html')
    else:
...
def correct_feedback_homework(request, num1):  # 老师批改学生反馈内容
    ...
    if request.method == 'POST':
        ...
        if userform.is_valid():
            ...
            return render_to_response('myApp/webpage/correct_feedback_homework_1.html')
    else:
...
def download(request, file_name):  # 下载作业
    ...
    if not os.path.isfile(file_path):  # 判断下载文件是否存在
        return render_to_response('myApp/webpage/download_1.html')
...
def check_all_student_homework(request, num, pindex):  # 查看所有学生主观题作业情况
    studentnamelist = Homework.objects.filter(homework_id=num).order_by('-id')
    homework1List = Homework1.objects.all()
    studentList = Student.objects.all().order_by('-id')
    paginator = Paginator(studentnamelist, 5)  # 实例化Paginator, 每页显示5条数据
    if pindex == "":  # django中默认返回空值，所以加以判断，并设置默认值为1
        ...
        int(pindex)
    page = paginator.page(pindex)  # 传递当前页的实例对象到前端
    return render_to_response('myApp/firstWeek/check_all_student_homework.html', {'studentnamelist': studentnamelist,
                                                                                    'homework_id': num,
                                                                                    'studentList': studentList,
                                                                                    'homework1List': homework1List,
                                                                                    "page": page})  # 'file_url':studentnamelist.feedback_homework.url})
